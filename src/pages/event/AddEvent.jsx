...
import { addEvent } from '../../redux/async/eventAsync';
...
const AddEvent = () => {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values, { resetForm }) => {
            ...
            if (addEvent.fulfilled.match(result)) {
                if (result.payload?.status === 1) {
                    toast.success(result.payload.message || 'Event added successfully');
                    resetForm();
                ...
                toast.error(result.payload || 'Event add failed');
            }
        }
    })
        ...
        <>
            ...
            <form onSubmit={formik.handleSubmit} className="space-y-8 mb-4">
                ...
                <div className="bg-white dark:bg-secondary border border-info dark:border-inputborder rounded-lg p-6 lg:col-span-2 hover:shadow-[0_8px_16px_rgba(0,0,0,0.2)] transition duration-300 hover:transform hover:-translate-y-[2px]">
                    <h1 className="text-3xl font-bold text-dark dark:text-lightwhite mb-6">Add Event</h1>
             ...
             rounded-md py-3 pl-5 pr-5 transition-colors duration-200 ease-in-out focus:outline-none"
                        ...
                        <div className='md:col-span-2'>
                            ...
                            {renderedImage && <div className="relative rounded-lg overflow-hidden mt-2">
                                <img src={renderedImage} alt="Preview" className="w-24 h-24 object-contain" />
                            </div>}
                        </div>
                    </div>
