...
const EditReview = () => {
    ...
    //======== Function to get the single details of the review ===========\\
    const getReviewDetails = async () => {
        try {
            const res = await apiJsonAuth.get(`/api/Admin/adminGetOneReview/${id}`);
            console.log(res);
            if (res.status === 200) {
                setReviewDetails(res.data.result);
            }
        } catch (error) {
    ...
    const formik = useFormik({
        initialValues: {
            userName: reviewDetails?.userName ? reviewDetails?.userName : '',
            rating: reviewDetails?.rating ? reviewDetails?.rating : '',
            description: reviewDetails?.description ? reviewDetails?.description : '',
        },
        validationSchema,
        ...
        onSubmit: async (values) => {
            try {
                ...
                setLoading(true);
                const res = await apiJsonAuth.put('/api/Admin/adminUpdateReview/'+id, payload);
